...
当前实现仍只适用于本机封闭验证；二维码会话已按拾音记账号隔离，但尚未加密持久化网易 Cookie。

## 8. 账号画像与二维码进展

当前版本已新增：

- 手机号 + MD5 密码的本地启动登录；遇到 `-460` 风控立即停止并提示二维码登录。
- 设置页二维码创建、扫码状态轮询和断开连接。
- Cookie 按拾音记账号保存在服务端内存，不进入前端响应或日志。
- 喜欢列表、播放记录、常听艺人与曲风/歌单标签形成账号熟悉度画像。
- 对排序窗口内的歌曲读取歌词和音乐百科，并缓存曲风、歌词密度与能量证据。

当前 Cookie 尚未加密写入 `music_connections`，因此 Web 服务重启后需要重新扫码。这是进入多用户测试前必须补齐的能力。
