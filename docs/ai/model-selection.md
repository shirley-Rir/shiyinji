...
```powershell
$env:EVAL_CASE_FILTER="study-01"
npm run eval:context
Remove-Item Env:EVAL_CASE_FILTER
```
...
2026-08-14 使用 `glm-4.7-flash`、关闭思考模式并按每条 1.8 秒间隔完成 32 条首轮评测。修正一处参考答案遗漏的合理同义词后：
...
这个结果达到首轮工程门槛，但仍需要三次重复评测和真实图片样本，才能判断长期稳定性。
