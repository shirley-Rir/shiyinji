...
| 账号连接 | 已接通 | 二维码登录为主；Cookie 只在服务端处理并加密持久化，不进入前端和模型 |
| 网易授权持久化 | 已完成 | Cookie 使用 AES-GCM 与账号 `user_id` 绑定加密，写入 `music_connections`；重启自动恢复，断开时按账号删除 |
| 拾音记用户系统 | 已完成 MVP | 邮箱验证码注册、密码登录、验证码登录、服务端会话与退出；画像、历史和反馈按账号隔离 |
| QQ 验证码邮件 | 已接通 | 本地通过 `smtp.qq.com:465` 使用 TLS 真实发送，SMTP 授权信息仅保存在忽略提交的环境变量中 |
| 数据与反馈 | 已接通 | D1 保存情境、推荐、曲目、播放事件、喜欢/不喜欢和方向反馈 |
| 自动验证 | 已完成基础项 | 类型检查、生产构建、SSR 测试和 54 条单元测试 |
...
| 网易授权依赖应用密钥 | 加密密钥丢失或变化后需重新扫码 | 密钥只存秘密环境并纳入服务器备份，不进入 Git |
| 尚无线上版权服务合同 | 不能公开商用 | 当前仅限本机封闭研发与验证，不部署为公开音乐服务 |
| 普通 VPS 尚无数据库适配器 | 当前 D1 版本不能直接作为通用 VPS 生产方案 | 认证领域与 API 已解耦；确认服务器后接 PostgreSQL 或单机 SQLite |
...
推荐算法正式评测暂缓，优先把 Web MVP 的账号与部署基础做完整：

1. 根据目标服务器确认 PostgreSQL 或单机 SQLite，增加对应数据适配器与生产迁移。
2. 配置正式域名、HTTPS 和 QQ SMTP，验证验证码邮件投递率、垃圾箱情况与发送限制。
3. 增加忘记密码、密码重置、全部设备退出和账号注销。
4. 对验证码增加 IP 与邮箱双维度限流，并补充异常登录审计。
5. 将网易云授权 Cookie 用应用密钥加密后持久化，支持服务重启和多实例。
6. 将当前同步式画像刷新迁移为后台任务，并增加增量刷新、失败重试和画像版本对比。
7. 用户系统稳定后，再恢复推荐测试集、草案回补和候选重排模型工作。
