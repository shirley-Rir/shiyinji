...
12. 当输入包含图片时，必须先提取能够区分图片的视觉场景指纹：场所类型、室内外、天气、光线/时段、空间开阔度、可观察活动、动静程度。把这些具体线索写入 environment 和 activity，禁止只写“图片情境”“户外”“风景”等泛化标签。
13. 纯图片无法证明用户真实内心情绪。此时 current_mood 表示画面氛围，target_mood 表示适合延续或调节该画面的聆听方向；两者仍必须给出具体中文标签，禁止输出“未知”“无法判断”。
14. 图片中的视觉动静和空间感必须影响 arousal 与 target_energy。例如安静书桌、雨窗、空旷山路、拥挤夜市、运动赛场应形成明显不同的数值与标签。
15. 图片与文字同时存在时，文字表达的真实心情和目标优先；图片用于补充环境、活动和氛围，不能覆盖用户明确表达。
`.trim();
...
9. context.source 为 image 或 text_image 时，context_evidence 是本轮推荐的强条件。先根据 activity、environment、画面氛围、空间感和 targetEnergy 确定声音策略，再用账号画像约束风格与熟悉度；账号画像不能把不同图片收敛成同一套歌曲。
10. 图片情境下，每条 search_lane 至少包含一个具体视觉线索，每首 draft_track 的 fit_reason 必须说明它对应的活动、环境或画面氛围。禁止仅用“治愈、放松、适合当下”等与任何图片都能匹配的泛化理由。
11. 对空旷自然、安静室内、拥挤城市、夜间灯光、运动场景等视觉差异，desired_sound 的能量、情绪、节奏或器乐方向至少有两项显著不同；同一账号画像下仍应产生不同草案。
`.trim();
